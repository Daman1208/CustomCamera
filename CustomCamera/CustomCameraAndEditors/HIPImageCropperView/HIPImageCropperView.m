...
@interface HIPImageCropperView ()<UIScrollViewDelegate>
...
- (id)initWithFrame:(CGRect)frame
    ...
    self.gridView = [[UIImageView alloc] initWithFrame:self.bounds];
    self.gridView.userInteractionEnabled = NO;
    [self.gridView setImage:[UIImage imageNamed:@"straighten-grid"]];
...
- (void)setOriginalImage:(UIImage *)originalImage
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.imageView setFrame:CGRectMake(0.0, 0.0, convertedImageSize.width,convertedImageSize.height)];
...
- (void)updateOverlay {
    ...
    if (_borderVisible) {
        ...
        [borderView.layer setBorderColor:[[[UIColor blackColor] colorWithAlphaComponent:0.5] CGColor]];
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    if(self.scrollBeginDraggingBlock)
        self.scrollBeginDraggingBlock();
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if(self.scrollEndDraggingBlock)
        self.scrollEndDraggingBlock();
}
